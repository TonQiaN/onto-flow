...
可以直接在画布上编辑节点引用的 Action，但编辑的对象始终是那个**共享的 Action 本身**，
改动对所有引用它的 Workflow 同时生效；只想改一处时，把该节点上的 Action
**复制为新 Action 并替换本节点引用**。
...
运行可被中途**取消**——已取消是区别于失败的独立终态（人为中止，不是出错）。

### 会话（Session）
一个 Action 节点执行时独占的一次 opencode 会话。会话是**用量归集的最小单位**：
它消耗的 token 与费用、调用过的工具、产生的事件，全部归属到它所属的那个节点。

### 思考强度（Reasoning Effort）
Action 的推理深度档位：`low / medium / high / max` 四档，默认 `max`。

### 修订（Revision）
实体（Workflow / Action / Skill / Tool / Object Type）每次保存时留存的一份完整定义历史。
可查看、与当前定义对比、回滚。修订是"这个实体一路怎么变过来的"的答案。

### 运行快照（Run Snapshot）
一次运行中，某个节点**实际使用的完整配置**的冻结副本（prompt、rule、各 Skill 与
Tool 全文、模型、思考强度、端口定义）。快照不随实体后续修改而改变，是"那次运行
到底跑了什么"的唯一答案，与[[修订]]互补：修订看实体演变，快照看历史运行。

### 标签（Tag）
跨全部库（Workflow / Action / Skill / Tool / Object Type）共用的分类维度。
**多归属**：一个实体可挂任意多个标签，不存在"它只能属于一处"的约束。
标签名可表达层级从属（如"采购"下的"集采"），浏览时呈现为可折叠的树，
但语义上仍是标签而非文件夹——同一实体可同时出现在多棵子树下。
