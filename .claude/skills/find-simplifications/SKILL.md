...
description: 在 OntoFlow 仓库里找有证据的简化候选——无生产消费者的路由 / 导出 / 表列、两处表示同一事实、无主人的投机通用性、只为未用 API 存在的防御、手写了依赖已有的东西、加了又拆的残留——写成 docs/simplifications/proposed/ 记录并顺带审计 rejected/。用户说「找找能简化什么」「清点冗余」「跑一轮简化」时用。
---
