...
GET 只有在汇总 Action 的持久事件中还能核对到该 Tool 的 `valid=true` 且错误为空回执时才返回成功结果；
仅写出结构合法 JSON、但没有实际完成校验调用的运行会返回契约错误。运行受理时还会把入口来源与
run 同步持久化；专用 GET 只读取专用 POST 发起的运行，同名工作流经通用运行接口启动也返回 404。
