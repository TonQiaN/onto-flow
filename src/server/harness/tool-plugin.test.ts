...
describe("Tool 包装生成", () => {
  it("包装含公名、描述、参数 schema、路径字面量、env 白名单键名与 execute 模块的 file URL", () => {
    const source = renderToolPlugin(sampleTool(), PATHS, "/runs/w/r/plugins/tool-sample-1.execute.ts", {
    ...
    expect(source).toContain('"parameters":{"type":"object","additionalProperties":false,"properties":{"label":{"type":"string","description":"标签"}},"required":["label"]}');
    ...
    expect(source).toContain('const ENV_KEYS = ["DEEPSEEK_API_KEY","TEAM_API_KEY","ONTOFLOW_DB_PATH"] as const;');
  ...
  it("声明了 output 与 timeoutMs 时原样进契约字面量", () => {
    ...
    expect(source).toContain('"output":{"type":"object","properties":{"ok":{"type":"boolean"}},"required":["ok"]}');
