...
function loadActionDtos(filterId?: string) {
  const actionRows = filterId
    ? db.select().from(actions).where(eq(actions.id, filterId)).all()
    : db.select().from(actions).orderBy(asc(actions.name)).all();
  if (actionRows.length === 0) return [];
  const ids = actionRows.map((a) => a.id);
  const portRows = db
    .select({
      id: actionPorts.id,
    ...
    .where(inArray(actionPorts.actionId, ids))
    .orderBy(asc(actionPorts.position))
    .all();
  const skillRows = db
    .select()
    .from(actionSkills)
    .where(inArray(actionSkills.actionId, ids))
    .orderBy(asc(actionSkills.position))
    .all();
  const toolRows = db
    .select()
    .from(actionTools)
    .where(inArray(actionTools.actionId, ids))
    .all();
...
export async function GET() {
  return handle(() => NextResponse.json(loadActionDtos()));
}
...
export async function POST(request: Request) {
  return handle(async () => {
    ...
    return NextResponse.json(loadActionDtos(actionId)[0]);
  });
}
