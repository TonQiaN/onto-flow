...
export async function POST(request: Request) {
  return handle(async () => {
    ...
    if (body.kind !== "action")
      return jsonError(400, "影响预览目前只支持 kind=action");
    if (typeof body.id !== "string" || body.id === "")
      return jsonError(400, "缺少 id 参数");
    if (!entityExists("action", body.id))
      return jsonError(404, "Action 不存在");
