...
/**
 * Tool 库列表页：左标签树 + 顶部搜索/排序/分页（状态同步 URL）+ 卡片列表。
...
function ToolsLibrary() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData((await res.json()) as ListEnvelope<ToolItem>);
    } catch {
  ...
  return (
    <>
      <LibraryLayout
        ...
        tree={<TagTree kind="tool" selected={tags} onChange={setTags} />}
        toolbar={
          <LibraryToolbar
            q={q}
            onQChange={setQ}
          ...
          {items.map((tool) => (
            ...
            >
              ...
              <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs">
                <TagBadges tags={tool.tags} onPick={(id) => setTags([id])} />
      ...
      {editor && (
        <ToolEditor
          ...
          initialTags={editor.mode === "edit" ? editor.tool.tags : []}
