...
const SPECS: TargetSpec[] = [
  ...
  },
  {
    key: "events",
    title: "事件明细",
    what: "删除 run_events 中早于 N 天的事件行（文本增量、工具调用、会话错误/空闲），并把够龄运行的轮次行与节点行上的输入输出与快照置空。",
    impact:
      "运行、节点与轮次的骨架（轮次、会话、起止、终态、出口、错误）与用量全部保留，事件日志变空、运行页的回放退化到轮次级；输入输出与快照会被置空，抽屉这两个页签只剩一句已清空的说明，轨迹面板退回显示技能 slug。",
    defaultDays: 30,
  },
  {
    key: "runs",
